...
$instance = $this->get_instance();
		...
		$objectToArray = (array)$submission;

	//echo '<pre>';
	//print_r($objectToArray);
	
//	die;
	 $sql_file = "SELECT `filename` FROM {files} WHERE `itemid`  = '".$objectToArray['id']."' AND `component` = 'assignsubmission_file' AND `filearea` = 'submission_files' AND `filename`!='.'";
     $list_file = $DB->get_records_sql($sql_file);
	 
	 foreach($list_file as $t=>$e)
		{
			$arr = explode(".",$e->filename);
			$ext = end($arr);
			if(strtolower($ext)=='ppt')
			{
				$has = 1;
			}
			unset($ext);
	...
	$sql_filetype = "SELECT `id` , `value` FROM {assign_plugin_config} WHERE `plugin`  = 'file' AND `name` = 'filetypeslist' AND `subtype` = 'assignsubmission' AND `assignment`='".$objectToArray['assignment']."'";
     $list_filetype = $DB->get_record_sql($sql_filetype);
	 
		

$filetype_arr = explode(",",$list_filetype->value);	


if($has==0 && (in_array(".ppt",$filetype_arr)==true || in_array(".PPT",$filetype_arr)==true || in_array(".pps",$filetype_arr)==true || in_array(".PPS",$filetype_arr)==true
|| in_array(".ppsx",$filetype_arr)==true || in_array(".PPSX",$filetype_arr)==true || in_array(".pttx",$filetype_arr)==true || in_array(".PPTX",$filetype_arr)==true || in_array(".pub",$filetype_arr)==true))
		{
			?>
  ...
  $( function() {
    $( "#dialog-confirm" ).dialog({
      ...
      modal: true,
      buttons: {
        "Okay I understand": function() {
          //$( this ).dialog( "close" );
		  window.location.href = "http://localhost/accit-moodle/accit/mod/assign/view.php?id=<?php echo $this->get_course_module()->id; ?>";
  ...
  <p>You must upload a PPT file as submission before you submit your assessment.</p>
</div>
			...
			//redirect('http://localhost/accit-moodle/accit/mod/assign/view.php?id='.$this->get_course_module()->id);
		}
...
//echo "nnnn".get_config('filetypeslist'); die;
//echo '<pre>';
//echo $this->get_course_module()->id; die;
        return $o;
    }
