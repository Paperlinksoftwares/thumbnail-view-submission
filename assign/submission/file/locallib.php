...
class assign_submission_file extends assign_submission_plugin
{
    ...
    /**
     * Full submission view – either a list or a gallery of thumbnails.
     *
     * @param stdClass $submission
     * @return string HTML for submission files
     */
    public function view(stdClass $submission)
    {
        ...
        if ($display === 'gallery') {
            ...
            $output .= html_writer::link($backurl, get_string('viewaslist', 'assignsubmission_file'));

            return $output;
        }

        // LIST MODE (default)
        $listhtml = $this->assignment->render_area_files(
            'assignsubmission_file',
            ASSIGNSUBMISSION_FILE_FILEAREA,
            $submission->id
        );
    ...
    /**
     * Override the “preview” icon link in the grading table
     * to force gallery mode.
     *
     * @param stdClass $submission The submission record.
     * @param int       $rownum     The row number (for returnparams).
     * @return string  HTML for the preview link.
     */
    public function view_header(stdClass $submission, $rownum)
    {
        global $PAGE, $OUTPUT;

        // Build the returnparams exactly as core does.
        $returnparams = [
            'rownum'       => $rownum,
            'useridlistid' => $this->assignment->get_useridlist_key_id(),
        ];

        // Build the URL parameters, adding display=gallery.
        $urlparams = [
            'id'           => $PAGE->cm->id,
            'sid'          => $submission->id,
            'gid'          => $submission->groupid,
            'plugin'       => $this->get_type(),              // “file”
            'action'       => 'viewplugin' . $this->get_subtype(), // “viewpluginassignsubmission”
        ...
        $icon = $OUTPUT->pix_icon('i/preview', get_string('view'), 'core', ['class' => 'icon']);

        return html_writer::link($url, $icon, ['class' => 'action-icon']);
    }
